...
public class SortDemo {
	...
	/**
	 * 插入排序的基本思想：在遍历数组的过程中，假设在序号 i 之前的元素即 [0..i-1] 都已经排好序，本趟需要找到 i 对应的元素 x 的正确位置
	 * k ， 并且在寻找这个位置 k 的过程中逐个将比较过的元素往后移一位，为元素 x “腾位置”，最后将 k 对应的元素值赋为 x ，
	...
	/**
	 * 
	 * 选择排序的基本思想:遍历数组的过程中，以 i 代表当前需要排序的序号，则需要在剩余的 [i…n-1] 中找出其中的最小值，然后将找到的最小值与 i
	 * 指向的值进行交换。 因为每一趟确定元素的过程中都会有一个选择最大值的子流程，所以人们形象地称之为选择排序。 选择排序的时间复杂度和空间复杂度分别为
	 * O(n2 ) 和 O(1) 。
	 * 
	 * @param int[] 未排序数组
	 * @return int[] 排完序数组
	...
	public static int[] sortSelect(int[] arr) {
		for (int i = 0; i < arr.length; i++) {
			...
			int miniPost = i;
			System.out.println("miniPost :" + miniPost);
			for (int m = i + 1; m < arr.length; m++) {
				System.out.println("arr[" + m + "] =" + arr[m]);
				if (arr[m] < arr[miniPost]) {
					miniPost = m;
					System.out.println("判断句： miniPost :" + miniPost + "  arr[" + miniPost + "] =" + arr[miniPost]);
				}
			}
			System.out.println("arr[" + i + "] =" + arr[i] + "  VS  " + "arr[" + miniPost + "] =" + arr[miniPost]);
	 ...
	 * 三：循环以上两步,直到 low == heigh, 这使才真正的找到了枢轴,分水岭. 返回这个位置,分水岭左边和右边的序列,分别再来递归。
	 * 
	 * @param int[] 未排序数组
	 * @return int[] 排完序数组
	 */
	 ...
	 * 到一起， 归并排序最重要的也就是这个“归并”的过程，归并的过程中需要额外的跟需要归并的两个数组长度一致的空间
	 * 
	 * @param int[] 未排序数组
	 * @return int[] 排完序数组
	 */
	 ...
	 * 希尔排序的思想是将一个大的数组“分而治之”，划分为若干个小的数组，以 gap 来划分，比如数组 [1, 2, 3, 4, 5, 6, 7, 8]，
	 * 如果以 gap = 2 来划分，可以分为 [1, 3, 5, 7] 和 [2, 4, 6, 8] 两个数组（对应的，如 gap = 3 ，
	 * 则划分的数组为： [1, 4, 7] 、 [2, 5, 8] 、 [3, 6] ）
	 * 然后分别对划分出来的数组进行插入排序，待各个子数组排序完毕之后再减小 gap 值重复进行之前的步骤，直至 gap = 1，
	...
	/**
	 * 堆排序算法 本质就是先构造一个大顶堆,parent比children大,root节点就是最大的节点
	...
	public int[] sortQuick_My(int[] array) {
		return sortQuick_My(array, 0, array.length - 1);
	}

	public int[] sortQuick_My(int[] array, int low, int high) {
		if (low < high) {
			int midPosition = partition_my(array, low, high);
			sortQuick_My(array, low, midPosition);
			sortQuick_My(array, midPosition + 1, high);
		}
		return array;
	}

	private int partition_my(int[] array, int low, int high) {
		int base = array[low];
		while (low < high) {
	...
	public static void main(String[] args) {
		int[] nums = {3, 9, 4, 8, 2, 6, 7, 5, 1};
		SortDemo demo = new SortDemo();
		// nums = sortInsert(nums);
		// nums = sortSelect(nums);
		// nums = sortBubble(nums);
//		nums = demo.sortMerge(nums);
//		 nums = demo.sortShell(nums);
		// nums = demo.sortQuick_My(nums);
		nums=demo.sortHeap(nums);
		for (int i : nums) {
			System.out.print(i + " ");
		}
	}
